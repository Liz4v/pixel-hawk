...
- `ingest.py` — `TileChecker` (tile monitoring orchestration, owns `httpx.AsyncClient`, query-driven project lookups via `TileProject`), `has_tile_changed()` (async tile download). `check_next_tile()` returns `list[int]` of affected project IDs (empty list if no tile selected or no projects linked)

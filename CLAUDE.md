...
- **Column helper.** `columns(cls)` in `db.py` introspects `dataclasses.fields()` and excludes names in the class's `_EXCLUDE_COLUMNS` frozenset (e.g. loaded relations like `owner`).
- **Dataclass conventions.** Each entity has a private `_from_row(row)` classmethod that maps a `sqlite3.Row` using `columns(cls)` for consistent column ordering. Public classmethods use explicit names (`get_by_id`, `get_or_none_by_discord_id`, `filter_by_owner_state`, `count_by_heat`) instead of generic `**kwargs` lookups. Instance methods: `save()` (with optional `update_fields`), `delete()`, `refresh_from_db()`. `save_as_new()` (on `ProjectInfo`) catches `sqlite3.IntegrityError` for random ID collision retry. Models with loaded relations (e.g. `ProjectInfo.owner`, `WatchMessage.project`) declare `_EXCLUDE_COLUMNS` to keep those fields out of SQL.

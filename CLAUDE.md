...
- `db.py` — database async context manager (`database()`), `SCHEMA` DDL (`CREATE TABLE IF NOT EXISTS ...`) applied on startup, module-level `_conn` plus `get_conn()` accessor, raw SQL helpers: `execute`, `execute_insert`, `fetch_one`, `fetch_all`, `fetch_val`, `fetch_int`. Uses `aiosqlite` with `sqlite3.Row` row factory; enables `PRAGMA journal_mode=WAL` and `PRAGMA foreign_keys=ON`; verifies write lock on startup via `_assert_db_writable()`.
- `entities.py` — dataclass models with raw SQL persistence: `Person` (user model with watched_tiles_count, active_projects_count, update_totals(), per-user quota limits), `ProjectState` IntEnum (ACTIVE/PASSIVE/INACTIVE/CREATING), `ProjectInfo` (owner_id FK, random ID via `save_as_new()`), `HistoryChange` (diff event log), `DiffStatus` IntEnum, `TileInfo` (tile metadata: coordinates, heat, timestamps, etag), `TileProject` (tile-project junction table), `GuildConfig` (per-guild bot configuration: required role name, quota ceilings), `WatchMessage` (persistent Discord watch messages, message_id as PK). Each dataclass exposes classmethods (`create`, `get`, `get_or_none`, `filter`, `all`, `count`) and instance methods (`save`, `delete`) wrapping SQL via `db` helpers.
...
There is no ORM. `models/db.py` owns the aiosqlite connection lifecycle, the full `SCHEMA` DDL, and a small set of query helpers; `models/entities.py` holds `@dataclass` models that call those helpers.

- **Connection lifecycle.** `async with database():` opens an `aiosqlite.Connection`, applies `PRAGMA journal_mode=WAL` + `PRAGMA foreign_keys=ON`, runs the `SCHEMA` `CREATE TABLE IF NOT EXISTS` script, verifies writability, and assigns the connection to the module-level `_conn`. The context manager saves/restores any prior `_conn` so nested `database()` calls (e.g. in tests) work correctly. Always access the connection via `get_conn()`, which asserts it's initialized.
- **Schema changes.** Edit `SCHEMA` in `db.py`. For brand-new tables or columns with a default, `CREATE TABLE IF NOT EXISTS` / `ALTER TABLE ... ADD COLUMN` on startup is sufficient. For destructive changes (rename, drop, type change) on an existing production DB, write a small one-shot migration helper and call it from `database()` behind a version check — but the project is early-stage, so prefer schema changes that are additive or can be applied by wiping the dev DB.
- **Query helpers.** Use `db.execute(sql, params)` for writes (auto-commits), `db.execute_insert(...)` to get the new `lastrowid`, `db.fetch_one(...)` for `sqlite3.Row | None`, `db.fetch_all(...)` for `list[sqlite3.Row]`, `db.fetch_val(...)` for a scalar (`int | float | str | None`), and `db.fetch_int(...)` for a `COUNT(*)`-style query that must return an `int`. Prefer `fetch_int` over `fetch_val` + `or 0` in count methods — the type checker requires the narrower return type.
- **Dataclass conventions.** Each entity has a private `_from_row(row)` classmethod that maps a `sqlite3.Row` to a dataclass instance, plus public classmethods (`create`, `get`, `get_or_none`, `filter`, `all`, `count`) and instance methods (`save`, `delete`). `save()` accepts an optional `update_fields` list for partial updates. A shared `_where_clause(kwargs)` helper builds `WHERE a = ? AND b = ?` fragments from keyword filters; FK lookups (e.g. `owner=person_obj`) are translated to `owner_id=person_obj.id` inside each model's classmethods.
- **Foreign keys and "related" fields.** There is no auto-loading of related objects. If you need `watch.project`, call a method like `WatchMessage.fetch_related_owner()` or populate the attribute explicitly (e.g. `get_watches_for_projects()` in `watch.py`). When a dataclass field holds an optional loaded relation, narrow it at the loop boundary with `assert watch.project is not None` and hoist to a local rather than asserting at each use site.

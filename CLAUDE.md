...
- **Key dependencies:** `loguru`, `pillow`, `httpx`, `aiosqlite` (raw SQL, no ORM), `humanize`, `discord.py`, `python-dotenv`
...
- `WatchMessage` (in `models/watch.py`) tracks persistent Discord messages that auto-update with project stats. Uses the Discord message snowflake as primary key (`message_id`). Unique constraint on `(project_id, channel_id)` enforces one watch per project per channel. FK to ProjectInfo with CASCADE delete.
...
- `db.py` — database async context manager (`database()`), `SCHEMA` DDL (`CREATE TABLE IF NOT EXISTS ...`) applied on startup, `MIGRATIONS` list with `_run_migrations()` for schema evolution (uses `PRAGMA user_version`), `transaction()` async context manager for atomic multi-statement writes, module-level `_conn` plus `get_conn()` accessor, raw SQL helpers: `execute`, `execute_insert`, `fetch_one`, `fetch_all`, `fetch_val`, `fetch_int`. Uses `aiosqlite` with `sqlite3.Row` row factory; enables `PRAGMA journal_mode=WAL` and `PRAGMA foreign_keys=ON`; verifies write lock on startup via `_assert_db_writable()`.
- `_sql.py` — private SQL helpers shared by entity modules: `_columns(cls)` (column names from dataclass fields, respecting `_EXCLUDE_COLUMNS`), `_where_clause(kwargs)` (WHERE clause builder). Not a public API.
- `person.py` — `Person` dataclass (user model with watched_tiles_count, active_projects_count, update_totals(), per-user quota limits), `BotAccess` IntFlag
- `project.py` — `ProjectInfo` dataclass (owner_id FK, random ID via `save_as_new()`), `ProjectState` IntEnum (ACTIVE/PASSIVE/INACTIVE/CREATING), `HistoryChange` (diff event log), `DiffStatus` IntEnum
- `tile.py` — `TileInfo` (tile metadata: coordinates, heat, timestamps, etag), `TileProject` (tile-project junction table)
- `guild.py` — `GuildConfig` (per-guild bot configuration: required role name, quota ceilings)
- `watch.py` — `WatchMessage` (persistent Discord watch messages, message_id as PK)
...
- `access.py` — admin and guild access service layer: `ErrorMsg` (user-facing exception), `imprint()` (first-admin bootstrap, only works on empty database), `coadmin()` (grant admin to another user, self-target delegates to `imprint()`), `grant_admin()` (admin grant, callers responsible for authorization), `set_guild_role()` (per-guild role configuration), `check_guild_access()` (role-based access gate with auto-creation, inherits guild quota ceilings), `check_dm_access()` (DM access gate, requires existing Person with ADMIN or ALLOWED), `get_user_quotas()` / `set_user_quotas()` / `get_guild_quotas()` / `set_guild_quotas()` (quota management with guild ceiling enforcement)
...
- `.github/workflows/deploy.yaml` — Auto-deploy on push to main via GitHub-hosted runner, Tailscale + SSH to production host (stop → pull → sync → start → verify). Schema lives in `db.py` and is applied by `CREATE TABLE IF NOT EXISTS` on startup — no separate migration step.
...
- Project state: `ProjectInfo` persists to SQLite via raw SQL (`await info.save()` issues an `UPDATE`, `save_as_new()` issues an `INSERT`). New projects must use `save_as_new()` (or `from_rect()` which calls it) to assign a random ID — do not use `ProjectInfo.create()` directly. `Project` objects are constructed on demand (not cached in memory) when `TileChecker` needs to diff affected projects. `Project.info` (not `.metadata`) holds the `ProjectInfo` instance. Business logic uses functional service layer: `metadata.process_diff(info, ...)` instead of `info.process_diff(...)`.
...
## Database layer: raw SQL + dataclasses

There is no ORM. `models/db.py` owns the aiosqlite connection lifecycle, the full `SCHEMA` DDL, migrations, transactions, and a small set of query helpers. Entity dataclasses live in per-entity modules (`models/person.py`, `models/project.py`, `models/tile.py`, `models/guild.py`, `models/watch.py`) with shared SQL helpers in `models/_sql.py`.

- **Connection lifecycle.** `async with database():` opens an `aiosqlite.Connection`, applies `PRAGMA journal_mode=WAL` + `PRAGMA foreign_keys=ON`, runs the `SCHEMA` `CREATE TABLE IF NOT EXISTS` script, runs migrations, verifies writability, and assigns the connection to the module-level `_conn`. The context manager saves/restores any prior `_conn` and `_in_transaction` so nested `database()` calls (e.g. in tests) work correctly. Always access the connection via `get_conn()`, which asserts it's initialized.
- **Transactions.** `async with db.transaction():` provides atomic multi-statement writes. Sets `_in_transaction = True`, commits on success, rolls back on exception. Inside a transaction, `db.execute()` skips per-statement auto-commit. Nesting is a no-op (the outer transaction controls commit/rollback). Use transactions for operations that must be atomic, like `link_tiles()` / `unlink_tiles()`.
- **Migrations.** `MIGRATIONS` in `db.py` is a list of `(description, sql)` tuples. `_run_migrations()` uses `PRAGMA user_version` to track which migrations have run. On first startup with an existing unversioned DB, it stamps `user_version` to `len(MIGRATIONS)` without running them (bootstrap). Subsequent migrations run incrementally. Add new migrations by appending to the list — never reorder or remove entries.
- **Schema changes.** For brand-new tables or columns with a default, add to `SCHEMA` (`CREATE TABLE IF NOT EXISTS` / column with default). For changes to existing columns or data transformations, add a migration to `MIGRATIONS`. The project is early-stage, so prefer additive schema changes.
- **Query helpers.** Use `db.execute(sql, params)` for writes (auto-commits unless in transaction), `db.execute_insert(...)` to get the new `lastrowid`, `db.fetch_one(...)` for `sqlite3.Row | None`, `db.fetch_all(...)` for `list[sqlite3.Row]`, `db.fetch_val(...)` for a scalar (`int | float | str | None`), and `db.fetch_int(...)` for a `COUNT(*)`-style query that must return an `int`. Prefer `fetch_int` over `fetch_val` + `or 0` in count methods — the type checker requires the narrower return type.
- **Shared SQL helpers (`_sql.py`).** `_columns(cls)` introspects `dataclasses.fields()` and excludes names in the class's `_EXCLUDE_COLUMNS` frozenset (e.g. loaded relations like `owner`). `_where_clause(kwargs)` builds `WHERE a = ? AND b = ?` fragments from keyword filters.
- **Dataclass conventions.** Each entity has a private `_from_row(row)` classmethod that maps a `sqlite3.Row` using `_columns(cls)` for consistent column ordering. Public classmethods use explicit names (`get_by_id`, `get_or_none_by_discord_id`, `filter_by_owner_state`, `count_by_heat`) instead of generic `**kwargs` lookups. Instance methods: `save()` (with optional `update_fields`), `delete()`, `refresh_from_db()`. `save_as_new()` (on `ProjectInfo`) catches `sqlite3.IntegrityError` for random ID collision retry. Models with loaded relations (e.g. `ProjectInfo.owner`, `WatchMessage.project`) declare `_EXCLUDE_COLUMNS` to keep those fields out of SQL.
- **Joined queries.** `_from_joined_row(row, prefix_map)` builds a dataclass from a JOIN result by extracting prefixed columns into a dict, avoiding try/except control flow. Used by `WatchMessage.filter_by_projects_with_owner()` and similar.
- **Batch queries.** `filter_by_ids(ids)` on `ProjectInfo` and `TileInfo` uses `WHERE id IN (?, ?, ...)` for batch lookups, replacing N+1 query patterns.
- **Foreign keys and "related" fields.** There is no auto-loading of related objects. If you need `watch.project`, use a joined query method or populate the attribute explicitly (e.g. `get_watches_for_projects()` in `watch.py`). When a dataclass field holds an optional loaded relation, narrow it at the loop boundary with `assert watch.project is not None` and hoist to a local rather than asserting at each use site.
- **Testing.** Tests run against a real SQLite file (temp dir) inside `async with database(db_path=...):`. The module-level `_conn` save/restore pattern means test fixtures can freely open and close databases without leaking state.
